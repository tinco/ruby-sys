...
fn rb_creating_and_running_fibers_works() {
    ...
    let run_test = || {
        unsafe {
            let test_fn = || { unsafe { fixnum::rb_int2inum(3) } };
            let test_fn_box = util::closure_box_ptr_create(test_fn);
            let fiber = fiber::rb_fiber_new(util::rbsys_closure_box_ptr_value, test_fn_box);
